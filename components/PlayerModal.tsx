...
function Pill({
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: "0.72rem", fontWeight: 700, color: colors.textSecondary, textTransform: "uppercase", letterSpacing: "0.07em" }}>
        {label}
      </span>
      <span style={{ fontSize: "1.5rem", fontWeight: 800, color: colors.textPrimary, letterSpacing: "-0.03em", lineHeight: 1 }}>
        {value}
      </span>
    </div>
  );
...
export function PlayerModal({ address, onClose }: PlayerModalProps) {
  ...
  const [oddsNum, oddsDen] = (oddsData as [bigint, bigint] | undefined) ?? [BigInt(0), BigInt(1)];
  const oddsPercent = oddsDen > BigInt(0)
    ? ((Number(oddsNum) / Number(oddsDen)) * 100).toFixed(2)
    : "0.00";

  const depositDisplay = depositWad != null
    ? Number(formatUnits(depositWad as bigint, 18)).toLocaleString(undefined, { maximumFractionDigits: 4 })
    : "0";
        ...
        <div style={{
          ...
          <div style={{
                ...
                <div style={{ display: "flex", gap: "8px", marginTop: "6px", flexWrap: "wrap" }}>
                  {isDepositor && (
                    <span style={{
                      ...
                      border: `1px solid ${colors.accent}44`, borderRadius: "999px",
                      padding: "2px 10px", fontSize: "0.72rem", fontWeight: 700,
                    }}>● In Pool</span>
                  )}
                  {hasPrize && (
                    <span style={{
                      backgroundColor: `${colors.reward}18`, color: colors.reward,
                      border: `1px solid ${colors.reward}44`, borderRadius: "999px",
                      padding: "2px 10px", fontSize: "0.72rem", fontWeight: 700,
                    }}>🏆 {myWins.length}x Winner</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <button onClick={handleClose} style={{
              ...
              display: "flex", alignItems: "center", justifyContent: "center",
              cursor: "pointer", fontSize: "1rem", color: colors.textSecondary,
          ...
          <div style={{
            ...
            <Pill label="Deposit" value={`${depositDisplay} MUSD`} accent={colors.primary} colors={colors} />
            <Pill label="Win Chance" value={`${oddsPercent}%`} accent={colors.accent} colors={colors} />
            <Pill label="Draws Won" value={myWins.length.toString()} accent={colors.secondary} colors={colors} />
            <Pill label="MEZO Won" value={`${formatToken(totalWon)}`} accent={colors.reward} colors={colors} />
          </div>
            ...
            {myWins.length === 0 ? (
              ...
              <div style={{ maxHeight: "220px", overflowY: "auto" }}>
                {[...myWins].reverse().map((draw, i) => (
                  <div key={draw.drawId.toString()} style={{
                    display: "grid", gridTemplateColumns: "64px 1fr 1fr",
                    padding: "11px 28px",
                    borderTop: i === 0 ? "none" : `1px solid ${colors.cardBorder}`,
                    alignItems: "center",
                  }}>
                    <span style={{ fontSize: "0.8rem", fontWeight: 700, color: colors.textTertiary }}>
                      #{draw.drawId.toString()}
                    </span>
                    <span style={{ fontSize: "0.88rem", fontWeight: 700, color: colors.reward }}>
                      {formatToken(draw.prizeNative)} MEZO
                    </span>
                    <span style={{ fontSize: "0.78rem", color: colors.textSecondary, textAlign: "right" }}>
